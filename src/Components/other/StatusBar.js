...
const StatusBar = ({ header, transitHistory }) => {
    ...
    // const cubicFunc = (x, u0, u1, u2, u3) => {
    //     return (u0 * (1 - x) ** 3) + (3 * u1 * ((1 - x) ** 2) * x) + (3 * u2 * (1 - x) * (x ** 2)) +( u3 * x ** 3)
    // }
			...
			<div className="status-bar">
				<div className="circles">
					{statusBar?.circleInfo?.shipped ? (
						<div className="status-circle filled" style={{"animationDelay": `${1 - (statusBar.barPercentage / 100)}s`}}>
							<BsCheck />
						</div>
					) : (
					...
					{statusBar?.circleInfo?.transit ? (
						<div className="status-circle filled" style={{animationDelay: `${(1 - (statusBar.barPercentage / 100)) + .33}s`}}>
							<BsCheck />
						</div>
					) : (
						<div className="status-circle unfilled"></div>
					...
					{statusBar?.circleInfo?.deliveryToday ? (
						<div className="status-circle filled" style={{animationDelay: `${(1 - (statusBar.barPercentage / 100)) + .66}s`}}>
							<BsCheck />
						</div>
					) : (
						<div className="status-circle unfilled"></div>
					...
					{statusBar?.circleInfo?.delivered ? (
						<div className="status-circle filled" style={{animationDelay: `${.99}s`}}>
							<BsCheck />
						</div>
					) : (
						<div className="status-circle unfilled"></div>
					...
					style={
						{
							backgroundImage: `linear-gradient(90deg,
									var(--clr-primary-200) ${statusBar.barPercentage / 2 - 4}%,
